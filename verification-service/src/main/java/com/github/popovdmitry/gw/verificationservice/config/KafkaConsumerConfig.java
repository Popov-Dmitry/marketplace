...
import com.github.popovdmitry.gw.verificationservice.dto.SellerDto;
...
public class KafkaConsumerConfig {
    ...
    @Bean
    public ConsumerFactory<String, SellerDto> consumerFactorySeller(){
        Map<String,Object> config = new HashMap<>();
        config.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, bootstrapServers);
        config.put(ConsumerConfig.GROUP_ID_CONFIG, sellerConsumerGroup);
        return new DefaultKafkaConsumerFactory<>(config, new StringDeserializer(), new JsonDeserializer<SellerDto>(SellerDto.class, false));
    }

    @Bean(name = "sellerInfoListener")
    public ConcurrentKafkaListenerContainerFactory<String, SellerDto> sellerInfoListener(){
